		...
		try
		{
			...
			else
			{
				...
				if ($class->hasMethod($method))
				{
					...
					$class->hasMethod('before') and $response = $class->getMethod('before')->invoke($this->controller_instance);

					$response === null and $response = $action->invokeArgs($this->controller_instance, $this->method_params);
